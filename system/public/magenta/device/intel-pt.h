...
// The PT register set.
// This is accessed via mtrace_control_thread, but basically it's a regset.
// TODO(dje): wip wip wip
typedef struct {
    uint64_t ctl;
    uint64_t status;
...
// mode values for IOCTL_IPT_SET_MODE
...
// TODO(dje): Would like a better name here than "data".
typedef struct {
    uint64_t capture_size;
} ioctl_ipt_buffer_data_t;

// get trace data associated with the buffer
// Inputs: trace buffer descriptor
// Outputs: ioctl_ipt_buffer_data_t
#define IOCTL_IPT_GET_BUFFER_DATA \
    IOCTL(IOCTL_KIND_DEFAULT, IOCTL_FAMILY_IPT, 7)
IOCTL_WRAPPER_INOUT(ioctl_ipt_get_buffer_data, IOCTL_IPT_GET_BUFFER_DATA,
                 uint32_t, ioctl_ipt_buffer_data_t);

typedef struct {
    uint32_t descriptor;
    uint32_t buffer_num;
} ioctl_ipt_buffer_handle_rqst_t;
...
IOCTL_WRAPPER_INOUT(ioctl_ipt_get_buffer_handle, IOCTL_IPT_GET_BUFFER_HANDLE,
                    ioctl_ipt_buffer_handle_rqst_t, mx_handle_t);
